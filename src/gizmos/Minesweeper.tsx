...
interface CellProps {
    ...
    readonly mineCount: Signal<number>;
}
            ...
            onClick={(e) => {
                ...
                if (e.button === 0) {
                    if (gameState.value === GameState.Pending) {
                        placeMines(grid, x, y, mineCount.value);
...
const Digits = ({ grid, gameState, mineCount }: DigitsProps) => {
    ...
    if (gameState.value === GameState.Pending) {
        remainingMines = mineCount.value;
    }
...
export const Minesweeper = ({ level }: GizmoProps) => {
    ...
    useSignalEffect(() => {
        ...
        if (gameState.value === GameState.Playing) {
            ...
            if (revealed >= gridSize * gridSize - mineCount.value) {
